    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="tableData.slice((pageno-1)*pageSize,pageno*pageSize)"  style="width: 100%">
          ...
          <el-table-column prop="ctId" label="编号" width="55px"></el-table-column>
          <el-table-column prop="ctName" label="名称" />
        </el-table>
...
<!--    <el-dialog v-model="dialogVisible" title="详情">
    ...
    <el-dialog v-model="dialogaddVisible" title="新增">
      <el-form  :model="entity" label-width="100px" >
        <el-form-item label="名称">
          <el-input v-model="entity.ctName" type="text"/>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    //加载所有数据
    load(){
      this.search.acType=1;
      this.axios.post("http://localhost:8088/coustomertype/show",this.search).then(v=>{
    ...
    submitForm(){
      ...
      var url="http://localhost:8088/coustomertype";
      if(this.index){
        url="http://localhost:8088/coustomertype/update"
      }
