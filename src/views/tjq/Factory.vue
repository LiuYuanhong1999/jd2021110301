  ...
  <div>
    ...
    <el-container>
      <el-header height="40px">
        <el-form size="mini" inline :model="search">
          <el-form-item label="名称：">
            <el-input type="text" v-model="search.factoryName"></el-input>
          </el-form-item>
          <el-form-item>
      ...
      <el-main>
        <el-table :data="tableData.slice((pageno-1)*pageSize,pageno*pageSize)"  style="width: 100%">
          ...
          <el-table-column prop="factoryId" label="编号" width="55px"></el-table-column>
    ...
    <el-dialog v-model="dialogaddVisible" title="新增">
      <el-form  :model="entity" label-width="100px" >
        <el-form-item label="名称">
          <el-input v-model="entity.factoryName" type="text"/>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    //加载所有数据
    load(){
      this.axios.post("http://localhost:8088/factory/show",this.search).then(v=>{
    ...
    //确定新增
    submitForm(){
      var url="http://localhost:8088/factory";
      if(this.index){
        url="http://localhost:8088/factory/update"
      }
